...
class AnalysisScopeResolver {
  ...
  AnalysisScope resolve(String? rawPath) {
    ...
    // ── feature (single-package) ── "lib/features/<feature>" or "features/<feature>"
    final singlePackageFeatureMatch = RegExp(r'^(?:lib/)?features/([^/]+)$').firstMatch(path);
    if (singlePackageFeatureMatch != null) {
      final featureName = singlePackageFeatureMatch.group(1)!;
      final appPackageName = _readPubspecName('.');
      return AnalysisScope(
        type: AnalysisScopeType.feature,
        scanRoot: path,
        caSearchRoot: 'lib',
  ...
  String resolve(AnalysisScope scope) {
    switch (scope.type) {
      ...
      case AnalysisScopeType.feature:
        ...
        return '$_baselinesRoot/features/${scope.featureName}.metrics.json';
