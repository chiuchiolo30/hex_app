...
# 0. Dependencias requeridas (Setup del proyecto)

## Propósito

Este proyecto arranca como un Flutter app vacío (`flutter create`). La arquitectura descripta en este documento depende de packages específicos que **no** están incluidos por defecto.

Antes de implementar la primera feature — o cualquier feature que necesite una capacidad que todavía no está en `pubspec.yaml` — el agente responsable (`architect.agent.md` / `feature-builder.agent.md`) DEBE revisar `pubspec.yaml` y agregar cualquier dependencia faltante de la tabla siguiente usando `flutter pub add <package>`.

## Stack estándar

| Package | Capa / Uso | Cuándo agregarlo |
|---|---|---|
| `flutter_bloc` | UI — manejo de estado con Cubit/Bloc | Requerido desde la primera feature |
| `equatable` | Domain/UI — igualdad por valor para Entities, States, Failures | Requerido desde la primera feature |
| `get_it` | DI — service locator | Requerido desde la primera feature |
| `dartz` | Domain/Data — `Either<Failure, T>`, `Unit` | Requerido desde la primera feature |
| `dio` | Data — cliente HTTP para datasources remotos | Requerido apenas una feature consuma una API REST |
| `go_router` | Routing | Requerido apenas la app tenga más de una pantalla / flujo de navegación |

## Reglas

- Este es el stack estándar del proyecto. No introducir librerías alternativas que resuelvan el mismo problema (ej. `riverpod`, `provider`, `result_dart`, `chopper`, `retrofit`) salvo decisión explícita del proyecto.
- `dartz` es el estándar del proyecto para `Either` / `Unit`. Todo subtipo de `Failure` debe extender `Equatable`.
- `dio` es el cliente HTTP estándar del proyecto. Los datasources remotos dependen de una instancia de `Dio` inyectada (o un adapter liviano sobre ella) — nunca instanciar `Dio` directamente dentro de un datasource.
- Los DTOs usan `fromJson` / `toJson` manuales (sin `freezed`, sin `json_serializable`) salvo decisión explícita del proyecto.
- Agregar una dependencia es parte del paso de Data/DI en la implementación de la feature. Debe quedar reflejado en `pubspec.yaml` y mencionarse explícitamente en el output de la feature (ej. "Dependencias agregadas: dio").
- No agregar una dependencia "por las dudas" — agregar solo lo que la feature actual realmente necesita.
