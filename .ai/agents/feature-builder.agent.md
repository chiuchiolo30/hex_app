...
Check `pubspec.yaml` against `.ai/context/architecture.md` section "0. Required Dependencies (Project Setup)".

* If a required package for this feature is missing, add it with `flutter pub add <package>`.
* Do not add packages outside the standard stack without explicit approval.
* Do not add a package the feature does not actually need.
...
List any package added to `pubspec.yaml` for this feature (or "None" if the stack already covered it).
